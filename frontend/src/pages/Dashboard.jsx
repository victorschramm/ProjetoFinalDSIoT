...
const Dashboard = () => {
  ...
  const [manutencaoPendente, setManutencaoPendente] = useState(0);
  ...
  const carregarDados = useCallback(async () => {
    try {
      ...
      const [ambientesResult, sensoresResult, leiturasResult, alertasResult, manutencaoResult] = await Promise.allSettled([
        getAmbientes(),
        getSensores(),
        getLeituras(),
        getAlertas(),
      ...
      if (manutencaoResult.status === 'fulfilled') {
        const pendentes = (Array.isArray(manutencaoResult.value) ? manutencaoResult.value : [])
          .filter(r => r.status === 'MANUTENCAO_PENDENTE').length;
        setManutencaoPendente(pendentes);
      } else {
        setManutencaoPendente(0);
      }
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Sincroniza o intervalo de refresh com o configurado no Monitoramento
        try {
          const { interval } = await getSensorInterval();
  ...
  // Auto-refresh: atualiza dados no mesmo intervalo do Monitoramento em Tempo Real
  useEffect(() => {
    const timer = setInterval(carregarDados, refreshInterval);
    return () => clearInterval(timer);
  }, [refreshInterval, carregarDados]);
