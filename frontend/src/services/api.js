...
export const login = async (email, password) => {
  try {
    ...
    if (!response.ok) throw { status: response.status, message: data.error || 'Erro ao fazer login' };
    return data;
  } catch (error) {
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      throw { status: 0, message: 'Servidor indisponível. Verifique se o backend está rodando.' };
    }
    throw error;
  }
};
...
export const register = async (userData) => {
  try {
    ...
    const data = await response.json();
    if (!response.ok) throw { status: response.status, message: data.error || 'Erro ao cadastrar usuário' };
    return data;
  } catch (error) {
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      throw { status: 0, message: 'Servidor indisponível. Verifique se o backend está rodando.' };
