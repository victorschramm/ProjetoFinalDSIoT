...
export const getLeituras = async () => {
  ...
  if (!response.ok) throw Object.assign(new Error(data.error || 'Erro ao listar leituras'), { status: response.status });
  return data;
};
...
export const getLeituraById = async (id) => {
  ...
  const data = await response.json();
  if (!response.ok) throw Object.assign(new Error(data.error || 'Erro ao obter leitura'), { status: response.status });
  return data;
};
...
export const getLeiturasBySensor = async (sensorId, limite = 50, tipo = null) => {
  ...
  const data = await response.json();
  if (!response.ok) throw Object.assign(new Error(data.error || 'Erro ao listar leituras do sensor'), { status: response.status });
  return data;
};
...
export const getLeiturasByPeriodo = async (inicio, fim, id_sensor = null) => {
  ...
  const response = await authFetch(endpoint);
  const data = await response.json();
  if (!response.ok) throw Object.assign(new Error(data.error || 'Erro ao listar leituras por período'), { status: response.status });
  return data;
};
...
export const createLeitura = async (payload) => {
  ...
  const data = await response.json();
  if (!response.ok) throw Object.assign(new Error(data.error || 'Erro ao registrar leitura'), { status: response.status });
  return data;
};
...
export const deleteLeitura = async (id) => {
  ...
  const data = await response.json();
  if (!response.ok) throw Object.assign(new Error(data.error || 'Erro ao deletar leitura'), { status: response.status });
  return data;
};
