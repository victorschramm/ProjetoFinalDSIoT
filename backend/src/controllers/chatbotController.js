...
module.exports = {
  async chat(req, res) {
    ...
    try {
      ...
      if (error.status === 429) {
        return res.status(429).json({ error: 'Limite diário de uso da IA (Groq) foi atingido. Tente novamente mais tarde ou aumente o limite em console.groq.com/settings/billing.' });
      }
      res.status(500).json({
